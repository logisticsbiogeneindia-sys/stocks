...
if not allowed_sheets:
    ...
    with tab4:
        ...
        if search_sheet == "Current Inventory":
            ...
            col1, col2, col3 = st.columns(3)
            with col1: date_range = st.date_input("Select Date Range", [])
            with col2: search_awb = st.text_input("Search by AWB Number").strip()
            with col3: search_customer = st.text_input("Search by Customer Name").strip()
