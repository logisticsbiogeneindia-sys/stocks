...
with tab4:
    ...
    description_col = find_column(search_df, ["Item Discription"])
    ...
    if search_sheet == "Current Inventory":
        ...
        if item_col:
            ...
            search_item = st.selectbox("Select Item Code", item_suggestions, index=0)

        if description_col:
            description_suggestions = search_df[description_col].dropna().unique()
            search_description = st.selectbox("Select Item Description", description_suggestions, index=0)
